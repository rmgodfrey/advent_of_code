...
const getInput = inputFetcher('/days/09/input.txt');
...
class Rope {
  constructor(length, start) {
    this.visited = [start],
    this.knots = Array(length).fill(start)
  }
}
...
const partOne = compose(
  instructions => instructions.reduce(move, new Rope(2, [0, 0])),
  ({ visited }) => visited.length
);

const partTwo = compose(
  instructions => instructions.reduce(move, new Rope(10, [0, 0])),
  ({ visited }) => visited.length
);
